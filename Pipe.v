...
module decode(Opout, regd, in, ir, allcoreen);
output reg `Op Opout;
output reg `REGNAME regd;
input wire allcoreen;
input wire `Op in;
input `WORD ir;
...
generate 
	...
	assign en[i] = processes[i].enwire;
		or(allcoreen ,allcoreen,processes[i].enwire);
...
endgenerate
decode MyDecode(DecOp, RegDst, s0Op, IReg, allcoreen);
...
always @(*) begin 
    NewPC = ((s1Op == `OpJumpf) && (allcoreen == 1)) ? s1ScVal :
